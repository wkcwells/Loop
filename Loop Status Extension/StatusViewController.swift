...
class StatusViewController: UIViewController, NCWidgetProviding {
    ...
    func update() -> NCUpdateResult {
        ...
        group.notify(queue: .main) {
            ...
            // KW - this got deleted in one of the merges????  Or something
        ...
        if let iob = context.activeInsulin {
            let formatter = NumberFormatter.glucoseFormatter(for: HKUnit.pintUS())  // KW random - we used to use eventualGlucose.unit, but if eventual glucose fails, then IOB will fail

            if let iobNumberString = formatter.string(from: NSNumber(value: iob)) {      // What if it's null?  Will probably just not show?  Test this.  Also: Use only one digit instead of 2?
